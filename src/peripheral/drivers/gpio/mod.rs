...
pub unsafe fn pin_function_set(pin: u32, function: PinFunction) -> () {
  ...
  let current_value: u32 = read_volatile(FSEL);

  let offset = base_bit * 3;
  let write_value: u32 = function.value() << offset;
  let mask_value: u32 = 0b111 << offset;

  write_volatile(FSEL, (current_value & (!mask_value)) | write_value);
}

pub unsafe fn pin_output_set(pin: u32) -> () {
  ...
  write_bit(SET, pin % 32, 1);
}

pub unsafe fn pin_output_clear(pin: u32) -> () {
